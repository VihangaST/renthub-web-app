...
# @ownerPricePredictions_bp.route('/predict', methods=['GET'])
# def predict():
#     try:
#         user_id = 2582
#         properties = Listing.query.filter_by(host_ID=user_id).all()
...
#         data=['Molenbeek-Saint-Jean','Entire home/apt','Entire rental unit',5.0,'1 bath',2.0,2.0]
    
#         neighbourhood = data[0]
#         room_type = data[1]
#         property_type = data[2]
#         accommodates = float(data[3])
#         bathrooms_text = data[4]
#         bedrooms = float(data[5])
#         beds = float(data[6])
        

#         # Encode categorical features using the label encoders
#         def encode_value(feature, value):
#             encoder = label_encoders.get(feature)  # Get the corresponding encoder from the loaded encoders
#             if encoder and value in encoder.classes_:
#                 return encoder.transform([value])[0]
#             return 0  # Default value if the value is not in the encoder classes

#         # Encode each categorical feature using the label encoders
...
#         return jsonify({"weekly_predictions": predictions})
...
# def predict():
#     try:
#         # User ID (Hardcoded for now, replace with request parameter if needed)
#         user_id = 2582
...
#             # Store predictions for each property
#             all_predictions[str(listing.id)] = {"weekly_predictions": predictions}

#         return jsonify({"PredictedPriceTrends": all_predictions})
